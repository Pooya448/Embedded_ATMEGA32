...
;	len -> R16,R17
;	i -> R18,R19
	LDI  R26,LOW(_equation)
	LDI  R27,HIGH(_equation)
	CALL _strlen
	MOVW R16,R30
	__GETWRN 18,19,0
_0x8:
...
; 0000 00DA                 strcat(operand2, cur_char);
	LDI  R30,LOW(_operand2)
	LDI  R31,HIGH(_operand2)
	RJMP _0x28
; 0000 00DB             } else {
_0xE:
; 0000 00DC                 strcat(operand1, cur_char);
	LDI  R30,LOW(_operand1)
	LDI  R31,HIGH(_operand1)
_0x28:
	ST   -Y,R31
	ST   -Y,R30
	LDI  R26,LOW(_cur_char)
	LDI  R27,HIGH(_cur_char)
	CALL _strcat
; 0000 00DD             }
; 0000 00DE         }
_0xD:
...
; 0000 00E1     op1 = (float) atof(operand1);
	LDI  R26,LOW(_operand1)
	LDI  R27,HIGH(_operand1)
	CALL _atof
	STS  _op1,R30
	STS  _op1+1,R31
	STS  _op1+2,R22
...
; 0000 00E2     op2 = (float) atof(operand2);
	LDI  R26,LOW(_operand2)
	LDI  R27,HIGH(_operand2)
	CALL _atof
	STS  _op2,R30
	STS  _op2+1,R31
	STS  _op2+2,R22
...
; 0000 00F3     ftoa(result, 3, final_string);
	...
	LDI  R30,LOW(3)
	ST   -Y,R30
	LDI  R26,LOW(_final_string)
	LDI  R27,HIGH(_final_string)
	CALL _ftoa
; 0000 00F4 }
	CALL __LOADLOCR4
	JMP  _0x20E0001
...
; 0000 00FE             strcat(equation, ch);
	LDI  R30,LOW(_equation)
	LDI  R31,HIGH(_equation)
	ST   -Y,R31
	ST   -Y,R30
	MOVW R26,R18
	CALL _strcat
...
; 0000 010E     memset(final_string, 0, sizeof(final_string));
	LDI  R30,LOW(_final_string)
	LDI  R31,HIGH(_final_string)
	ST   -Y,R31
	ST   -Y,R30
	LDI  R30,LOW(0)
	ST   -Y,R30
